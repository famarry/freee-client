...
class SectionParams implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setCode($code)
    {
        if (!is_null($code) && (mb_strlen($code) > 20)) {
            throw new \InvalidArgumentException('invalid length for $code when calling SectionParams., must be smaller than or equal to 20.');
        }
        if (!is_null($code) && (!preg_match('/^[0-9a-zA-Z_-]+$/', $code))) {
            throw new \InvalidArgumentException("invalid value for $code when calling SectionParams., must conform to the pattern /^[0-9a-zA-Z_-]+$/.");
        }
     ...
     * Sets parent_id.
     *
     * @param int|null $parent_id 親部門ID (プレミアムプラン、法人スタンダードプラン（および旧法人ベーシックプラン）以上)
     *
     * @return self
     */
