...
class SegmentTag implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setCode($code)
    {
        if (!is_null($code) && (mb_strlen($code) > 20)) {
            throw new \InvalidArgumentException('invalid length for $code when calling SegmentTag., must be smaller than or equal to 20.');
        }
        if (!is_null($code) && (!preg_match('/^[0-9a-zA-Z_-]+$/', $code))) {
            throw new \InvalidArgumentException("invalid value for $code when calling SegmentTag., must conform to the pattern /^[0-9a-zA-Z_-]+$/.");
        }
