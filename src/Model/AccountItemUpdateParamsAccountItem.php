...
class AccountItemUpdateParamsAccountItem implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setCode($code)
    {
        if (!is_null($code) && (mb_strlen($code) > 20)) {
            throw new \InvalidArgumentException('invalid length for $code when calling AccountItemUpdateParamsAccountItem., must be smaller than or equal to 20.');
        }
        if (!is_null($code) && (!preg_match('/^[0-9a-zA-Z_-]+$/', $code))) {
            throw new \InvalidArgumentException("invalid value for $code when calling AccountItemUpdateParamsAccountItem., must conform to the pattern /^[0-9a-zA-Z_-]+$/.");
        }
    ...
    public function setCorrespondingExpenseId($corresponding_expense_id)
    {
        if (($corresponding_expense_id < 1)) {
            throw new \InvalidArgumentException('invalid value for $corresponding_expense_id when calling AccountItemUpdateParamsAccountItem., must be bigger than or equal to 1.');
        }
    ...
    public function setAccumulatedDepAccountItemId($accumulated_dep_account_item_id)
    {
        if (!is_null($accumulated_dep_account_item_id) && ($accumulated_dep_account_item_id < 1)) {
            throw new \InvalidArgumentException('invalid value for $accumulated_dep_account_item_id when calling AccountItemUpdateParamsAccountItem., must be bigger than or equal to 1.');
        }
