...
/**
 * ExpenseApplicationParentApprovableRequestUpdateParams Class Doc Comment.
 *
 * @category Class
 *
 * @author   OpenAPI Generator team
 ...
 */
class ExpenseApplicationParentApprovableRequestUpdateParams implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * @var string
     */
    protected static $openAPIModelName = 'expenseApplicationParentApprovableRequestUpdateParams';
    ...
    {
        if (($company_id < 1)) {
            throw new \InvalidArgumentException('invalid value for $company_id when calling ExpenseApplicationParentApprovableRequestUpdateParams., must be bigger than or equal to 1.');
        }
     ...
     * Sets parent_id.
     *
     * @param int $parent_id 各種申請ID - 経費申請に関連付ける各種申請IDを指定します - 閲覧権限のある承認済みの既存各種申請IDのみ指定ができます - 既に経費申請に紐付いている各種申請との関連は解除されます - 経費申請に紐付く各種申請を解除する場合はnullを指定してください
     *
     * @return self
     */
    ...
    {
        if (($parent_id < 1)) {
            throw new \InvalidArgumentException('invalid value for $parent_id when calling ExpenseApplicationParentApprovableRequestUpdateParams., must be bigger than or equal to 1.');
        }
